...
case "$(uname -s)" in
	...
	Darwin)
		...
		DOCKER_USER=serialtcp
		DOCKER_GROUP=serialtcp
...
mkdir -p "${XDG_CACHE_HOME}/serialtcp"
...
docker run \
	...
	${TTY} \
	--interactive \
	--volume "${GIT_ROOT}:${DOCKER_HOME}/serialtcp" \
	--volume "${XDG_CACHE_HOME}/serialtcp:${DOCKER_XDG_CACHE_HOME}/serialtcp" \
	--env "XDG_CACHE_HOME=${DOCKER_XDG_CACHE_HOME}" \
	${GO_ENV_ARGS} \
	--workdir ${DOCKER_HOME}/serialtcp \
	${DOCKER_IMAGE} \
